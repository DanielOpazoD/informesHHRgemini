...
2. O bien, abre **Configuración → IA** dentro de la aplicación e ingresa tu clave de la API de Gemini; la clave solo se guarda en tu navegador. Si la clave proviene de Google Cloud Console, ingresa también el número del proyecto para adjuntarlo en la cabecera `X-Goog-User-Project` y asegúrate de que tu cuenta tenga el rol **serviceusage.serviceUsageConsumer** en ese proyecto. Desde el mismo modal puedes indicar opcionalmente el modelo de Gemini; si lo dejas vacío usaremos `gemini-1.5-flash` y comprobaremos automáticamente en qué versión (`v1` o `v1beta`) está disponible antes de mandar cualquier solicitud. Si escribes otro modelo también lo validaremos antes del primer uso, y aún puedes forzar la versión agregando `@v1` o `@v1beta` al final.
...
4. (Opcional) Ajusta el modelo. Por defecto usamos `gemini-1.5-flash`, pero puedes escribir cualquier otro (`gemini-pro`, `gemini-1.5-pro`, etc.). El sistema probará automáticamente los endpoints `v1` y `v1beta` hasta encontrar el que realmente existe para tu cuenta; si ninguno responde, el asistente te indicará el error exacto y sugerirá pasos para habilitarlo.
