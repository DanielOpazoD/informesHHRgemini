...
/**
 * Verifica la disponibilidad de un modelo de Gemini para la cuenta/API key dada, explorando heurísticas de endpoints (v1/v1beta).
 */
...
/**
 * Función principal para generar contenido utilizando la API de Gemini (Streaming no soportado aquí).
 * Maneja automáticamente los reintentos por Límite de Tarifa (Rate Limit - 429) y el enrutamiento API inteligente (v1/v1beta).
 *
 * @param req Configuración de la solicitud (clave API, modelo, contenidos, etc.)
 * @returns La respuesta de la API ya convertida a un objeto del tipo T.
 * @throws {Error | GeminiModelUnavailableError} Si la API falla o el modelo es inaccesible.
 */
