...
const PatientInfo: React.FC<PatientInfoProps> = ({
    ...
    return (
        <div className="sec" id="sec-datos">
            <div className="subtitle" contentEditable={isStructureEditing} suppressContentEditableWarning>Información del Paciente</div>
            <div id="patientGrid">
                    ...
                    {patientFields.filter(f => !f.isCustom).map((field) => {
                        ...
                        return (
                            <div key={field.id || originalIndex} className="patient-field-row">
                                <div className="lbl" contentEditable={isStructureEditing} suppressContentEditableWarning onBlur={e => onPatientLabelChange(originalIndex, e.currentTarget.innerText)}>{field.label}</div>
                                <input
                                    type={field.type}
                                    className="inp"
                                    id={field.id}
